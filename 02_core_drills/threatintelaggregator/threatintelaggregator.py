# CDD
# FUNCTION: ingest an unknown number of feeds and return a clean list
...
def aggregate_intel(*vendors: list) -> list:
    ...
    for vendor in vendors: # iterate through each vendor using a for loop
        for threat in vendor: # extract lists
             if threat['sev'] == "High": # extract IP's with a "high" severity
                 ip_threats.append(threat['ip']) # append to a new list
